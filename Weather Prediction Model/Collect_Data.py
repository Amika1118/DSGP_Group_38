...
def collect_global_warming_indicators() -> pd.DataFrame:
    ...
    for idx, year in enumerate(years):
        ...
        pdo_cycle = np.sin(2 * np.pi * years_since_2010 / 20)
        ...
        for month in range(1, 13):
            ...
            climate_record = {
                ...
                'carbon_budget_remaining_gt': round(500 - years_since_2010 * 42, 0),
                'radiative_forcing_wm2': round(2.3 + years_since_2010 * 0.03, 3),
    ...
    # Calculate trends with proper window handling
    climate_df['temp_anomaly_trend_10yr'] = climate_df['global_temp_anomaly'].rolling(
        window=120, min_periods=1, center=False
    ).mean()

    climate_df['co2_trend_5yr'] = climate_df['co2_ppm'].rolling(
        window=60, min_periods=1, center=False
    ).mean()
